...
function sessionState(session) {
  if (session.importKind === "spreadsheet" && session.spreadsheetScan && !session.spreadsheetAnalysis) {
    return { phase: "spreadsheet-scanned", next: ["submit_spreadsheet_analysis", "scan_spreadsheet"] };
  }
  if (session.importKind === "spreadsheet" && session.spreadsheetAnalysis && !session.spreadsheetCompilation) {
    return { phase: "spreadsheet-semantics-ready", next: ["build_spreadsheet_layout"] };
  }
  if (session.importKind === "spreadsheet" && session.spreadsheetCompilation && !session.spreadsheetVerified) {
    return { phase: "spreadsheet-compiled", next: ["verify_spreadsheet", "render", "submit_spreadsheet_analysis", "scan_spreadsheet"] };
  }
  if (session.importKind === "spreadsheet" && session.spreadsheetVerified) {
    return { phase: "spreadsheet-verified", next: ["accept_import", "cancel_import"] };
  }
  if (!session.plan) {
    return { phase: "no-plan", next: ["scan_spreadsheet", "create_plan", "open_plan", "open_sample"] };
  }
  if (session.importKind === "reference" && session.referenceMode && !session.referenceScan) {
    return { phase: "source-loaded", next: ["scan_reference"] };
  }
  if (session.importKind === "reference" && session.referenceScan && !session.referenceAnalysis) {
    return { phase: "scanned", next: ["submit_reference_analysis"] };
  }
  if (session.importKind === "reference" && session.referenceAnalysis && !session.referenceCompilation) {
    return { phase: "semantics-ready", next: ["replace_layout"] };
  }
  if (session.importKind === "reference" && session.referenceCompilation && !session.referenceVerified) {
    return { phase: "compiled", next: ["verify_reference"] };
  }
  if (session.importKind === "reference" && session.referenceVerified) {
    return { phase: "verified", next: ["accept_import", "cancel_import"] };
  }
  return {
  ...
  server.registerTool("editor_capabilities", {
    ...
    reference: {
      ...
      supports: ["yerel piksel tarama", "otomatik koltuk merkezi ve sıra segmenti",
        "çoklu koltuk kümesi", "yerel OCR/focal aday raporu", "staged preview",
      ...
      limitations: [
        ...
        "Codex koltuk saymaz ve koordinat üretmez; yalnız tarayıcının rowId ve focal adaylarına anlam verir.",
    ...
    spreadsheet: {
      ...
      extensions: [".xls", ".xlsx"],
      families: ["named-range-plan", "canvas-sheet-plan", "flat-list"],
      layouts: ["normalized", "source", "ring"],
      ...
      rules: [
        ...
        "Excel yalnız tek görünür çalışma sayfası içermelidir; çok sayfalı dosya planı değiştirmeden reddedilir.",
        "source varsayılandır ve tek ölçekle sayfa oranlarını korur. normalized yalnız açıkça seçilen şematik yerleşimdir; kaynak geometrisi sayılmaz.",
        "Global konumsuz listelerde ring açıkça seçilmelidir. Mimari doğruluk veya genel %99 başarı iddiası üretilmez.",
      ],
    },
    validation: {
