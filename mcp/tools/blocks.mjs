...
export function registerBlockTools(server, session, z) {
  ...
  server.registerTool("add_block", {
    ...
    inputSchema: {
      ...
      gates: z.array(z.string()).optional().describe("Planda kapı varsa, yeni bloğun bağlı olacağı kapı etiketleri/kimlikleri"),
    },
  }, async (a) => {
    ...
    for (const g of a.gates || []) {
      const s = (plan.shapes || []).find((x) => x.type === "door" && (x.id === g || x.label === g));
      if (!s) throw new Error(`Kapı bulunamadı: ${g}`);
      gatePatch.push(s.id);
    }
    const shapes = gatePatch.length
      ? (plan.shapes || []).map((s) => (gatePatch.includes(s.id)
        ? { ...s, blocks: [...new Set([...(s.blocks || []), b.id])] } : s))
      : (plan.shapes || []);
    return { ...plan, blocks: [...plan.blocks, b], shapes };
