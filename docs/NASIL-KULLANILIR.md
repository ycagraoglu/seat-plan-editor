...
**Kimlik.** `SEAT_EDITOR_JWKS_URL` verilirse tüm API istekleri Bearer JWT
ile doğrulanır ve kiracı `tenant_id` claim'inden alınır. JWKS yoksa sunucu
varsayılan olarak erişimi reddeder. Yalnız yerel geliştirmede açıkça
`SEAT_EDITOR_AUTH_DEV_BYPASS=1` verilirse localhost isteklerinde eski
`x-tenant-id` başlığı kullanılabilir; JWKS açıksa bu başlık dikkate alınmaz.

```js
window.__SEAT_EDITOR_AUTH__ = {
  apiBase: "/api",
  tokenProvider: async () => oturum.jwt,
};
```
...
| Excel planı | Oturma planını tek görünür sayfada içeren `.xls/.xlsx` dosyasını ver; MCP dört adımlı Excel akışını kullanır |
